...
module.exports = function (from, to, state, dryRun, cb) {
  // adding a argument without changes to pre-generate code
  ...
  fs.readFile(geonamesrc, 'utf-8', (err, buffer) => {
    ...
    return head(from)
      ...
      .then((lastModified) => {
        if (dryRun) return cb(null, { lastModified, cachedDate: (new Date(parseInt(data[from]))) }) // this is just for check
  ...
  function downloadZIP (config, cb) {
    ...
    return get(from)
      ...
      .then(([headers, response]) => {
        ...
        return fs.writeFile(geonamesrc, ini.stringify(config), cb)
      }).catch(cb)
  }
}
