...
function Generator(where, state, args) {
  ...
  this.where = where ? where : cli.config.get('dir');
  this.state = state;
...
Generator.prototype.extractGeonameData = function(cb) {
  ...
  let geonamePath = path.join(cli.config.get('cache'), 'geonames')
...
Generator.prototype.saveMongoDBCountryData = function(countries, cb) {
  let where = path.join(this.where, './docker/database/data')
...
Generator.prototype.saveELKCountryData = function(cb) {
  let where = path.join(this.where, './docker/elasticsearch/data')
  return mkdirp(where).then(() => {
    // this is smaller and safer to hold the data that can be then
    // literally mapped, pun intended, into the cities for
    let countries = []
    ...
    let data = countries.map((item) => JSON.stringify(item, '', null));
...
Generator.prototype.saveELKCityData = function(cb) {
  ...
  return mkdirp(where).then(() => {
    ...
    let data = cities.map((item) => JSON.stringify(item, '', null));
    data.push(''); // For the ndjson
...
Generator.prototype.printUpdated = function(cb) {
  ...
  if (cities) {
    var action = `updated ${cities} cit${(cities > 1 ? 'ies' : 'y')}`
    actions.push(action)
  }
  if (countries) {
    var action = `added ${countries} countr${(countries > 1 ? 'ies' : 'y')}`
    actions.push(action)
  }
