...
const citySchema = require('./generate/schema/city')
const download = require('./generate/network/download')
const alternate = require('./generate/schema/alternate')

const generate = module.exports = {}

generate.usage = new Usage('generate', '\nGenerate the geolocation data', [
  'geo generate'
], '[--dir=.]\n')
...
generate.run = function (argv, cb) {
  // const prefix = require('./utils/prefix')(cli.config.get('prefix'))
  //
  // console.log(prefix('mongo', 'cities.json'), prefix('elasticsearch', 'cities.json'))
  //
  // cb()
  ...
  return new Generator(argv[0], state, argv).run(cb)
}

function Generator (where, state, args) {
  // since we use prefix now to generate the file
  // paths, where needs to be stored in dir
  cli.config.set('dir', where || cli.config.get('dir'))

  this.args = args
  this.command = check()

  this.prefix = prefix(cli.config.get('prefix'))
...
Generator.prototype.run = function (_cb) {
  var result
  var cb
  if (_cb) {
    cb = function () {
...
Generator.prototype.newTracker = function (name) {
...
Generator.prototype.finishTracker = function (name, cb) {
...
Generator.prototype.cloneCountries = function (cb) {
...
Generator.prototype.pullCountries = function (cb) {
...
Generator.prototype.extractGeonameData = function (cb) {
  /**
   * Now we should download the zip files for the cities and alternative names
   *
   * Here we should also have a .geonamesrc ini file that holds the URL used
...
Generator.prototype.handleCountries = function (cb) {
  ...
  //
  // move this here so we can run the save mongodb and elk
  // separately
  this.countries = new Map(countries.map((country) => [country.codes.cca2, country.name]))

  if ([check.COMMANDS.MONGODB, check.COMMANDS.ALL].includes(this.command)) {
    steps.push([this, this.saveMongoDBCountryData, countries])
  }

  if ([check.COMMANDS.ELASTICSEARCH, check.COMMANDS.ALL].includes(this.command)) {
    steps.push([this, this.saveELKCountryData])
  }

  return chain(steps, cb)
}

Generator.prototype.saveMongoDBCountryData = function (countries, cb) {
  const where = this.prefix('mongodb', 'countries.json')
  const dirname = path.dirname(where)

  return mkdirp(dirname).then(() => {
    fs.writeFile(where, JSON.stringify(countries, '', null), cb)
  }).catch(cb)
}

Generator.prototype.saveELKCountryData = function (cb) {
  const where = this.prefix('elasticsearch', 'countries.ndjson')
  const dirname = path.dirname(where)
  ...
  return mkdirp(dirname).then(() => {
    ...
    fs.writeFile(where, data.join('\n'), cb)
  }).catch(cb)
}

Generator.prototype.handleCites = function (cb) {
  const steps = []
  ...
  if ([check.COMMANDS.MONGODB, check.COMMANDS.ALL].includes(this.command)) {
    steps.push([this, this.saveMongoDBCityName])
  }

  if ([check.COMMANDS.ELASTICSEARCH, check.COMMANDS.ALL].includes(this.command)) {
    steps.push([this, this.saveELKCityData])
  }

  return chain(steps, cb)
}

Generator.prototype.saveMongoDBCityName = function (cb) {
  const where = this.prefix('mongodb', 'cities.json')
  const dirname = path.dirname(where)

  return mkdirp(dirname).then(() => {
    fs.writeFile(where, JSON.stringify(this.cities, '', null), cb)
  }).catch(cb)
}

Generator.prototype.saveELKCityData = function (cb) {
  const where = this.prefix('elasticsearch', 'cities.ndjson')
  const dirname = path.dirname(where)
...
Generator.prototype.readAlternateNames = function (where, cb) {
...
Generator.prototype.readCityNames = function (where, cb) {
...
Generator.prototype.printUpdated = function (cb) {
