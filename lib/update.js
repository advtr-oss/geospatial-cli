...
Updater.prototype.run = function(_cb) {
  ...
  // console.log(command);
  //
  // const where = path.join(this.where, './docker/elasticsearch/data/geolocation.ndjson');
  // const response = tmp('json');
  //
  // console.log(response);
  // bulk(cli.config.get('elasticsearchuri'), where,  response, cb);

  // const where = path.join(this.where, './docker/elasticsearch/data/geolocation.ndjson');
  // // add a `--elasticsearchuri=` option here
  // bulk(cli.config.get('elasticsearchuri'), where, cb);
  // // exec(where, path.join(where, './geolocation.ndjson'), cb);
  //
  // fs.unlink(where, cb);

  // whereAndExec(path.join(this.where, './docker/database/data'), {
  //   uri: cli.config.get('mongouri'),
  //   collection: cli.config.get('collection')
  // }, cb);
